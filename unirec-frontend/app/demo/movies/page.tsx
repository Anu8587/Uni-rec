...
export default function MoviesDemo() {
  ...
  const loadDataset = async () => {
    const data: Item[] = await fetch("/data/movies.json").then((r) => r.json());
    setItems(data);
    await uploadItems(data);
    pushLog(`Loaded ${data.length} movie items`);
  };
        ...
        <header className="flex items-center justify-between mb-6">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-purple-400">
              Movies · OTT · Context-aware
            </p>
            <h1 className="text-3xl md:text-4xl font-semibold mt-1">
              Personalised Streaming Experiences
            </h1>
            <p className="text-slate-400 text-sm mt-2 max-w-xl">
              Simulate a streaming platform: interact with titles and watch the
              engine infer mood and suggest the next best show to watch.
            </p>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-slate-400">
                {items.length ? `${items.length} titles loaded` : "No titles yet"}
              </span>
            </div>
            <div className="rounded-2xl border border-slate-800 bg-black/40 p-3">
              {items.length > 0 ? (
                ...
                <p className="text-sm text-slate-500">
                  Click{" "}
                  <span className="text-purple-300">“Load Movie Items”</span> to
                  ingest the OTT catalog.
                </p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="rounded-2xl border border-purple-700/40 bg-[#050014] p-3 min-h-[220px]">
              {recommendationItems.length > 0 ? (
                ...
                <p className="text-sm text-slate-500">
                  Watch, like or click on a few titles, then hit{" "}
                  <span className="text-purple-300">“Get Recommendations”</span>.
                </p>
              )}
            </div>
          </div>
