...
export default function BlogsDemo() {
  ...
  const loadDataset = async () => {
    const data: Item[] = await fetch("/data/news.json").then((r) => r.json());
    setItems(data);
    await uploadItems(data);
    pushLog(`Loaded ${data.length} news/blog items`);
  };
        ...
        <header className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-2 max-w-xl">
              Discover how the same engine can drive contextual recommendations
              for articles, news and long-form content.
            </p>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-slate-400">
                {items.length ? `${items.length} articles loaded` : "No content yet"}
              </span>
            </div>
            <div className="rounded-2xl border border-slate-800 bg-black/40 p-3">
              {items.length > 0 ? (
                ...
                <p className="text-sm text-slate-500">
                  Click{" "}
                  <span className="text-purple-300">“Load Blog/News Items”</span>{" "}
          ...
          <div>
            ...
            <div className="rounded-2xl border border-purple-700/40 bg-[#050014] p-3 min-h-[220px]">
              {recommendationItems.length > 0 ? (
                ...
                <p className="text-sm text-slate-500">
                  Interact with a few posts, then hit{" "}
                  <span className="text-purple-300">“Get Recommendations”</span>{" "}
                  to see personalised reading suggestions.
                </p>
              )}
            </div>
          </div>
